...
- **Two sources**: Skills in `.agents/skills/` may be **native** (tracked in this repo, e.g. `xp-*`, `test-doubles-first`, `cwv-improvement-planner`, `lean-ai-adoption-coach`, `team-ownership`) or **from skill-factory** (symlinked from the [skill-factory](https://github.com/saski/skill-factory) repo via `./pull-and-sync-skills.sh`). Consider all skills in `.agents/skills/` when matching the user's request.
